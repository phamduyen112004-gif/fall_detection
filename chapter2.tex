...
\subsection{Hạn chế của mạng hồi quy (RNN/LSTM)}
\label{sec:lstm_limitations}

Mạng LSTM (\textit{Long Short-Term Memory}) được thiết kế để khắc phục hạn chế của RNN cơ bản trong việc nắm bắt phụ thuộc dài hạn thông qua cơ chế cổng (\textit{gate mechanism}) gồm forget gate, input gate và output gate. Mỗi cell duy trì một cell state $C_t$ được cập nhật có chọn lọc qua các time step:

\begin{equation}
C_t = f_t \odot C_{t-1} + i_t \odot \tilde{C}_t
\end{equation}

trong đó $f_t$ và $i_t$ lần lượt là forget gate và input gate, $\tilde{C}_t$ là candidate cell state mới.

Tuy nhiên, LSTM vẫn gặp phải vấn đề \textbf{triệt tiêu đạo hàm} (\textit{vanishing gradient}) khi chuỗi thời gian quá dài. Đạo hàm của loss theo hidden state ở time step đầu tiên bị nhân liên tục bởi các ma trận trọng số qua nhiều bước thời gian, dẫn đến giá trị cực nhỏ và mạng không thể học được các phụ thuộc dài hạn hiệu quả. Điều này đặc biệt bất lợi cho bài toán phát hiện té ngã vì một cú ngã thường trải qua nhiều giai đoạn: mất thăng bằng → rơi tự do → va chạm → nằm bất động, kéo dài hàng chục khung hình.

Ngoài ra, LSTM xử lý chuỗi tuần tự theo cơ chế recurrence, không thể song song hóa (\textit{parallelization}) hiệu quả trên GPU hiện đại. Điều này làm tăng độ trễ xử lý và không phù hợp cho ứng dụng thời gian thực.
