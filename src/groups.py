...
def assign_groups(clip_paths: list[str | Path]) -> list[str]:
    """
    Assign group IDs to a list of clip paths.

    Args:
        clip_paths: List of clip file paths.

    Returns:
        List of group IDs (same length as input).
    """
    return [group_id_from_clip_path(Path(p)) for p in clip_paths]
